"""Provider-neutral state models used by the standalone LangGraph runtime."""
...
class ToolRequest(BaseModel):
    """Tool call plus server-trusted customer identity from session state."""
    call: ToolCall
    customer_email: str


class PendingPurchase(BaseModel):
    """Purchase details exposed while the graph waits for a human."""
    track_ids: list[int]
    description: str | None = None


class ApprovalDecision(BaseModel):
    """Human decision recorded before graph execution resumes."""
    approved: bool
